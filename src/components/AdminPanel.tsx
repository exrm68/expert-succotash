        ...
        <div className="max-w-6xl mx-auto">
          ...
          {activeTab === 'settings' && (
            <div className="space-y-6">
              ...
              {/* ── 18+ SECTION CONTROL ── */}
              <div className="bg-[#18181b] border border-pink-500/20 rounded-[20px] p-5 space-y-4">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-lg">🔞</span>
                  <h3 className="text-sm font-black text-pink-400">18+ Section Control</h3>
                </div>

                {/* Home Banner URL */}
                <div>
                  <label className="text-xs text-zinc-400 mb-1 block">Home Banner URL (YouTube 16:9 size image)</label>
                  <input
                    type="text"
                    value={(settings as any).adultBannerUrl || ''}
                    onChange={e => setSettings((prev: any) => ({ ...prev, adultBannerUrl: e.target.value }))}
                    placeholder="https://... (16:9 thumbnail image link)"
                    className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-pink-500"
                  />
                  {(settings as any).adultBannerUrl && (
                    <img
                      src={(settings as any).adultBannerUrl}
                      className="mt-2 w-full aspect-video object-cover rounded-xl border border-white/10 max-w-[280px]"
                    />
                  )}
                  <p className="text-[10px] text-zinc-600 mt-1">এই image টা home page এ 18+ banner হিসেবে দেখাবে</p>
                </div>

                {/* Tutorial Banner */}
                <div>
                  <label className="text-xs text-zinc-400 mb-1 block">"কিভাবে দেখবেন" Banner URL (Inside 18+ page)</label>
                  <input
                    type="text"
                    value={(settings as any).adultTutorialBanner || ''}
                    onChange={e => setSettings((prev: any) => ({ ...prev, adultTutorialBanner: e.target.value }))}
                    placeholder="https://... (tutorial banner image)"
                    className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-pink-500"
                  />
                </div>
                <div>
                  <label className="text-xs text-zinc-400 mb-1 block">Tutorial Click Link</label>
                  <input
                    type="text"
                    value={(settings as any).adultTutorialLink || ''}
                    onChange={e => setSettings((prev: any) => ({ ...prev, adultTutorialLink: e.target.value }))}
                    placeholder="https://t.me/... (tutorial video link)"
                    className="w-full bg-[#242427] border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-pink-500"
                  />
                </div>

                {/* Hide/Show toggle */}
                <div className="flex items-center justify-between p-3 bg-[#242427] rounded-xl">
                  <div>
                    <p className="text-sm font-bold text-white">18+ Section Hide করো</p>
                    <p className="text-xs text-zinc-500">On করলে home page থেকে পুরো 18+ section লুকাবে</p>
                  </div>
                  <button
                    onClick={() => setSettings((prev: any) => ({ ...prev, adultSectionHidden: !(prev as any).adultSectionHidden }))}
                    className={`relative w-12 h-6 rounded-full transition-all ${(settings as any).adultSectionHidden ? 'bg-red-500' : 'bg-zinc-600'}`}
                  >
                    <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${(settings as any).adultSectionHidden ? 'left-6' : 'left-0.5'}`} />
                  </button>
                </div>
              </div>
